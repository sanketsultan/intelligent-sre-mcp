...
class TestRunner:
    ...
    def test_prometheus_connection(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{API_URL}/prom/query", params={"query": "up"}, timeout=TIMEOUT
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def test_kubernetes_connection(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{API_URL}/k8s/pods", params={"namespace": NAMESPACE}, timeout=TIMEOUT
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def test_anomaly_detection(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{API_URL}/detection/anomalies", params={"namespace": NAMESPACE}, timeout=TIMEOUT
            )
            if response.status_code == 200:
                data = response.json()
                ...
                self.print_info(
                    f"  CPU: {cpu}, Memory: {memory}, Restarts: {restarts}, Pending: {pending}"
                )
                return data
            else:
    ...
    def test_pattern_recognition(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{API_URL}/detection/patterns", params={"namespace": NAMESPACE}, timeout=TIMEOUT
            )
            if response.status_code == 200:
                data = response.json()
                ...
                self.print_info(
                    f"  Recurring: {recurring}, Cyclic: {cyclic}, Exhaustion: {exhaustion}, Cascading: {cascading}"
                )
                return data
            else:
                self.print_failure(
                    f"Pattern recognition returned status code {response.status_code}"
                )
                return {}
        except Exception as e:
    ...
    def test_correlation_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_info(
                    f"  Restart-Event: {restart_event}, CPU-Event: {cpu_event}, Memory-OOM: {memory_oom}"
                )
                return data
            else:
                self.print_failure(
                    f"Correlation analysis returned status code {response.status_code}"
                )
                return {}
        except Exception as e:
    ...
    def test_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_info(
                    f"  Total Correlations: {correlations.get('total_correlations', 0)}"
                )
                return data
            else:
                self.print_failure(
                    f"Comprehensive analysis returned status code {response.status_code}"
                )
                return {}
        except Exception as e:
    ...
    def test_metric_spike_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                ...
                params={"query": query, "duration": "1h", "spike_multiplier": 2.0},
                timeout=TIMEOUT,
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def test_prometheus_targets(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(f"{API_URL}/prom/targets", timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
    ...
    def run_all_tests(self):
        ...
        print(
            f"{Colors.BOLD}Starting test run at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.END}\n"
        )
        ...
        self.test_metric_spike_detection()
        ...
        if self.failed == 0:
            ...
            print(
                f"{Colors.YELLOW}{Colors.BOLD}⚠ Some tests failed. Please review the output above.{Colors.END}\n"
            )
