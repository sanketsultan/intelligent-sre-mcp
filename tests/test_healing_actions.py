...
class TestHealingActions(unittest.TestCase):
    ...
    def test_02_restart_pod_dry_run(self):
        ...
        pods_response = requests.get(
            f"{API_URL}/k8s/pods", params={"namespace": TEST_NAMESPACE}, timeout=TEST_TIMEOUT
        )
        ...
        response = requests.post(
            ...
            params={"namespace": TEST_NAMESPACE, "pod_name": test_pod, "dry_run": True},
    ...
    def test_03_delete_failed_pods_dry_run(self):
        ...
        response = requests.post(
            ...
            params={"namespace": TEST_NAMESPACE, "dry_run": True},
            timeout=TEST_TIMEOUT,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_05_drain_node_dry_run(self):
        ...
        nodes_response = requests.get(f"{API_URL}/k8s/nodes", timeout=TEST_TIMEOUT)
    ...
    def test_07_rollback_deployment_dry_run(self):
        ...
        response = requests.post(
            ...
            params={"namespace": TEST_NAMESPACE, "deployment_name": "prometheus", "dry_run": True},
            timeout=TEST_TIMEOUT,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_08_cordon_node_dry_run(self):
        ...
        response = requests.post(
            ...
            params={"node_name": test_node, "dry_run": True},
            timeout=TEST_TIMEOUT,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_10_healing_action_history(self):
        ...
        response = requests.get(
            f"{API_URL}/healing/action-history", params={"hours": 24}, timeout=TEST_TIMEOUT
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        ...
        if data.get("by_action_type"):
            ...
            for action_type, stats in data["by_action_type"].items():
                print(
                    f"    {action_type}: {stats['total']} (✓{stats['success']} ✗{stats['failed']})"
                )
    ...
    def test_15_action_stats(self):
        ...
        response = requests.get(
            f"{API_URL}/learning/action-stats", params={"hours": 24}, timeout=TEST_TIMEOUT
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_13_actual_delete_failed_pods(self):
        ...
        print(
            "  To test manually: Create a failing pod, then call delete-failed-pods without dry_run"
        )
