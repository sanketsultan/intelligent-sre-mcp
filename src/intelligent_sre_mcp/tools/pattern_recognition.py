...
class PatternRecognizer:
                    ...
                    if restarts >= self.recurring_failure_threshold:
                        ...
                        patterns.append(
                            Pattern(
                                ...
                                first_seen=(
                                    datetime.now() - self._parse_duration(self.time_window)
                                ).isoformat(),
                                last_seen=datetime.now().isoformat(),
                                confidence=confidence,
                                recommendation="Investigate pod logs and check for resource limits, configuration issues, or application errors",
                ...
                try:
                    ...
                    patterns.append(
                        Pattern(
                            ...
                            affected_resources=[{"deployment": deployment, "namespace": ns}],
    ...
    def analyze_all_patterns(self, namespace: Optional[str] = None) -> Dict[str, Any]:
        ...
        high_confidence = sum(
            len([p for p in patterns if p.confidence >= 0.8]) for patterns in all_patterns.values()
        )
    ...
    def _generate_insights(self, all_patterns: Dict[str, List[Pattern]]) -> List[str]:
        ...
        if recurring:
            insights.append(
                f"⚠️ Found {len(recurring)} pods with recurring failures - these need immediate attention"
            )
        ...
        if exhaustion:
            insights.append(
                f"📈 Detected {len(exhaustion)} potential memory leaks or resource exhaustion trends"
            )
        ...
        if cascading:
            insights.append(
                f"🔥 {len(cascading)} cascading failure patterns detected - possible systemic issue"
            )
        ...
        if cyclic:
            insights.append(
                f"🔄 {len(cyclic)} cyclic patterns found - likely scheduled workloads or periodic processes"
            )
        ...
        if deployment:
            insights.append(
                f"🚀 {len(deployment)} deployment rollout issues - check recent changes"
            )
