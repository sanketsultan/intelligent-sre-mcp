...
class HealingActions:
    ...
    def restart_pod(self, namespace: str, pod_name: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            action_id = self.limiter.record_action(
                action_type, namespace, pod_name, True, "Pod deleted for restart"
            )
            ...
            action_id = self.limiter.record_action(
                action_type, namespace, pod_name, False, error_msg
            )
    ...
    def delete_failed_pods(
        self, namespace: str, label_selector: Optional[str] = None, dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            failed_pods = [
                pod for pod in pods.items if pod.status.phase in ["Failed", "Succeeded", "Unknown"]
            ]
            ...
            allowed, reason = self.limiter.can_perform_action(
                action_type, affected_resources=len(failed_pods)
            )
            if not allowed:
                return {
                    "success": False,
            ...
            action_id = self.limiter.record_action(
                action_type,
                namespace,
                f"{len(deleted_pods)} pods",
                True,
                f"Deleted {len(deleted_pods)} failed pods",
            )
            ...
            logger.error(error_msg)
            action_id = self.limiter.record_action(
                action_type, namespace, "multiple", False, error_msg
            )
    ...
    def evict_pod_from_node(
        self, namespace: str, pod_name: str, dry_run: bool = False, grace_period_seconds: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            action_id = self.limiter.record_action(
                action_type, namespace, pod_name, True, "Pod evicted"
            )
    ...
    def drain_node(
        ...
        try:
            ...
            allowed, reason = self.limiter.can_perform_action(
                action_type, affected_resources=len(evictable_pods)
            )
            if dry_run:
                ...
                return {
                    ...
                    "pods": [
                        f"{pod.metadata.namespace}/{pod.metadata.name}" for pod in evictable_pods
                    ],
            ...
            for pod in evictable_pods:
                ...
                try:
                    ...
                    failed.append({"pod": f"{pod_namespace}/{pod_name}", "error": e.reason})

            action_id = self.limiter.record_action(
                action_type, "-", node_name, True, f"Evicted {len(evicted)} pods from node"
            )
    ...
    def scale_deployment(
        self, namespace: str, deployment_name: str, replicas: int, dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if dry_run:
                logger.info(
                    f"[DRY RUN] Would scale {namespace}/{deployment_name} from {current_replicas} to {replicas}"
                )
                return {
                    "success": True,
                    "action": action_type,
            ...
            logger.info(
                f"Scaled deployment {namespace}/{deployment_name} from {current_replicas} to {replicas}"
            )
    ...
    def rollback_deployment(
        ...
        try:
            ...
            if dry_run:
                ...
                logger.info(
                    f"[DRY RUN] Would rollback {namespace}/{deployment_name} to {revision_msg}"
                )
                return {
                    "success": True,
                    "action": action_type,
            ...
            revision_msg = f"revision {revision}" if revision else "previous revision"
            action_id = self.limiter.record_action(
                action_type, namespace, deployment_name, True, f"Rolled back to {revision_msg}"
            )
    ...
    def cordon_node(self, node_name: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            action_id = self.limiter.record_action(
                action_type, "-", node_name, True, "Node cordoned"
            )
    ...
    def uncordon_node(self, node_name: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            action_id = self.limiter.record_action(
                action_type, "-", node_name, True, "Node uncordoned"
            )
    ...
    def get_action_history(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": round(successful_actions / total_actions * 100, 1)
            if total_actions > 0
            else 0,
