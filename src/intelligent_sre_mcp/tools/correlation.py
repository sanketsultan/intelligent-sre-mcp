...
class CorrelationEngine:
    ...
    def __init__(self, prometheus_url: str = None):
        self.prom_url = (
            prometheus_url or os.getenv("PROMETHEUS_URL", "http://prometheus:9090")
        ).rstrip("/")
    ...
    def correlate_restarts_with_events(self, namespace: Optional[str] = None) -> List[Correlation]:
        ...
        query = (
            f'increase(kube_pod_container_status_restarts_total{{namespace="{namespace}"}}[15m]) > 0'
            if namespace
            else "increase(kube_pod_container_status_restarts_total[15m]) > 0"
        )

        restart_result = self._query_prometheus(query)

        if restart_result.get("status") == "success" and restart_result.get("data", {}).get(
            "result"
        ):
                        ...
                        if recent_events:
                            ...
                            correlations.append(
                                Correlation(
                                    ...
                                    recommendation=self._generate_restart_recommendation(
                                        recent_events
                                    ),
    ...
    def correlate_cpu_spikes_with_events(
        ...
        query = (
            f'sum(rate(container_cpu_usage_seconds_total{{namespace="{namespace}"}}[5m])) by (pod, namespace) * 100 > 70'
            if namespace
            else "sum(rate(container_cpu_usage_seconds_total[5m])) by (pod, namespace) * 100 > 70"
        )
                ...
                try:
                    ...
                    if events_data.get("status") == "success" and events_data.get("events"):
                        ...
                        for event in events_data["events"][:20]:
                            ...
                            if time_diff <= self.correlation_window and event.get("reason", "") in [
                                "ScalingReplicaSet",
                                "SuccessfulCreate",
                                "Scheduled",
                            ]:
                        ...
                        if deployment_events:
                            correlations.append(
                                Correlation(
                                    ...
                                    description="High CPU usage correlated with recent deployment activity",
                                    confidence=0.7,
                                    timestamp=datetime.now().isoformat(),
                                    primary_signal={
                                        "type": "metric",
    ...
    def correlate_memory_with_oom_events(
        self, namespace: Optional[str] = None
    ) -> List[Correlation]:
        ...
        query = (
            f'(sum(container_memory_working_set_bytes{{namespace="{namespace}"}}) by (pod, namespace) / sum(container_spec_memory_limit_bytes{{namespace="{namespace}"}}) by (pod, namespace)) * 100 > 80'
            if namespace
            else "(sum(container_memory_working_set_bytes) by (pod, namespace) / sum(container_spec_memory_limit_bytes) by (pod, namespace)) * 100 > 80"
        )
    ...
    def _generate_restart_recommendation(self, events: List[Dict]) -> str:
        ...
        if any("OOM" in r or "Killed" in r for r in reasons):
            ...
            return (
                "Health probe failures - adjust probe settings or fix application health endpoints"
            )
    ...
    def _generate_correlation_insights(self, correlations: List[Dict]) -> List[str]:
        ...
        if "cascading" in by_type:
            insights.append(
                f"🔥 Cascading failures detected - {len(by_type['cascading'])} incident(s) affecting multiple pods"
            )

        if "metric_to_event" in by_type:
            insights.append(
                f"📊 {len(by_type['metric_to_event'])} metric anomalies correlated with Kubernetes events"
            )

        if "event_to_metric" in by_type:
            insights.append(
                f"⚡ {len(by_type['event_to_metric'])} performance changes linked to recent deployments or scaling"
            )
