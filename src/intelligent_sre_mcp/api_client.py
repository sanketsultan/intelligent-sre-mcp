...
def get_agent_activity(hours: int = 24, limit: int = 50) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            response = client.get(
                f"{API_URL}/learning/agent-activity", params={"hours": hours, "limit": limit}
            )
            response.raise_for_status()
            return str(response.json())
    except httpx.HTTPError as e:
...
def list_problems(hours: int = 24, limit: int = 50) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            response = client.get(
                f"{API_URL}/learning/problems", params={"hours": hours, "limit": limit}
            )
            response.raise_for_status()
            return str(response.json())
    except httpx.HTTPError as e:
...
def list_tool_invocations(hours: int = 24, limit: int = 100) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            response = client.get(
                f"{API_URL}/learning/tool-invocations", params={"hours": hours, "limit": limit}
            )
            response.raise_for_status()
            return str(response.json())
    except httpx.HTTPError as e:
...
@mcp.tool()
def k8s_get_events(
    namespace: str = None, resource_type: str = None, resource_name: str = None
) -> str:
    """
...
def detect_metric_spike(query: str, duration: str = "1h", spike_multiplier: float = 2.0) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT * 2) as client:
            params = {"query": query, "duration": duration, "spike_multiplier": spike_multiplier}
...
def restart_pod(namespace: str, pod_name: str, dry_run: bool = False) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            params = {"namespace": namespace, "pod_name": pod_name, "dry_run": dry_run}
...
def delete_failed_pods(namespace: str, label_selector: str = None, dry_run: bool = False) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            params = {"namespace": namespace, "dry_run": dry_run}
            if label_selector:
...
def rollback_deployment(
    namespace: str, deployment_name: str, revision: int = None, dry_run: bool = False
) -> str:
    """
...
def cordon_node(node_name: str, dry_run: bool = False) -> str:
    ...
    try:
        with httpx.Client(timeout=TIMEOUT) as client:
            params = {"node_name": node_name, "dry_run": dry_run}
...
def record_action_outcome(
    action_id: int, outcome: str, resolution_time_seconds: float = None, notes: str = None
) -> str:
    """
